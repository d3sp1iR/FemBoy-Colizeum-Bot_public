...
def generate_adventure_report(femboy_name):
    ...
    for i in range(num_events):
        ...
        # Используем глобальную переменную для шанса
        if random.random() < ITEM_CHANCE:
            possible_items = [item for item in AVAILABLE_ADVENTURE_ITEMS if random.random() < item["chance"]]
            if possible_items:
                ...
                print(f"✅ Предмет найден в приключении: {found_item['name']} (ID: {found_item['id']})")
    
    print(f"📦 Итоговые найденные предметы: {[item['name'] for item in found_items]}")
...
def apply_item_bonuses(conn, femboy_id, found_items):
    ...
    print(f"🔄 Применяем бонусы для фембоя {femboy_id}")
    print(f"📦 Найдено предметов: {len(found_items)}")
    
    for item in found_items:
        print(f"🎯 Обрабатываем предмет: {item['name']} (ID: {item['id']})")
        ...
        if existing_item:
            print(f"⚠️ Предмет {item['name']} уже есть в инвентаре, пропускаем")
            continue
        ...
        if item["type"] == "weapon":
            ...
            print(f"⚔️ Бонус к оружию: +{item['value']}")
        elif item["type"] == "armor":
            total_armor_bonus += item["value"]
            print(f"🛡️ Бонус к броне: +{item['value']}")
    ...
    if total_weapon_bonus > 0 or total_armor_bonus > 0:
        ...
        print(f"📊 Обновляем характеристики: weapon_atk={femboy_stats['weapon_atk']} -> {new_weapon_atk}, armor_def={femboy_stats['armor_def']} -> {new_armor_def}")
        
        cur.execute("""
            UPDATE femboys SET weapon_atk=?, armor_def=? WHERE id=?
        """, (new_weapon_atk, new_armor_def, femboy_id))
    else:
        print("ℹ️ Нет бонусов для применения")
...
def complete_adventure(adv_id, femboy_id, chat_id):
    ...
    try:
        ...
        if adventure and adventure["completed"] == 1:
            print(f"⚠️ Приключение {adv_id} уже завершено, пропускаем")
            conn.close()
            return None
        ...
        print(f"🎯 Завершаем приключение для фембоя: {femboy['name']} (ID: {femboy_id})")
